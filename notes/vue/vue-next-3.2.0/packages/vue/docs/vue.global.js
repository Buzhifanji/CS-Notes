var Vue = (function(exports) {
    ...
    const normalizeSlotValue = (value) => isArray(value) ?
        value.map(normalizeVNode) : [normalizeVNode(value)];
    ...
    */
    ...
    function createAppAPI(render, hydrate) {
        ...
        return function createApp(rootComponent, rootProps = null) {
            ...
            const app = (context.app = {
                ...
                _instance: null,
                version,
                get config() { // 读取  app.config(全局)
                    return context.config;
                },
                set config(v) { // 更改 app.config(全局)
                    {
                        warn$1(`app.config cannot be replaced. Modify individual options instead.`);
                    }
                },
                use(plugin, ...options) { // 定义 app.use(全局)  使用插件的方法
                ...
                mixin(mixin) { // 定义 app.mixix(全局)  混入逻辑的方法
                    {
                ...
                component(name, component) { // 定义 app.component(全局)  存储组件的方法
                    {
                ...
                directive(name, directive) { // 定义 app.directive(全局)  存错指令的方法
                    {
                ...
                mount(rootContainer, isHydrate, isSVG) { // 定义 app.mount(全局)  挂载组件方法
                    if (!isMounted) {
                        // 创建根组件的 vnode
                ...
                provide(key, value) { // 定义 app.provide(全局)  依赖注入存储对应数据的方法
    ...
    const normalizeRef = ({ ref }) => {
        return (ref != null ?
            isString(ref) || isRef(ref) || isFunction(ref) ? { i: currentRenderingInstance, r: ref } :
            ref :
            null);
    };
    ...
    function cloneVNode(vnode, extraProps, mergeRef = false) {
        ...
        const cloned = {
            ...
            ref: extraProps && extraProps.ref ? // #2078 in the case of <component :is="vnode" ref="extra"/>
                ...
                mergeRef && ref ?
                isArray(ref) ?
                ref.concat(normalizeRef(extraProps)) : [ref, normalizeRef(extraProps)] :
                normalizeRef(extraProps) : ref,
            scopeId: vnode.scopeId,
            slotScopeIds: vnode.slotScopeIds,
            children: patchFlag === -1 /* HOISTED */ && isArray(children) ?
                children.map(deepCloneVNode) : children,
            target: vnode.target,
            ...
            patchFlag: extraProps && vnode.type !== Fragment ?
                ...
                ?
                16 /* FULL_PROPS */ :
                patchFlag | 16 /* FULL_PROPS */ : patchFlag,
    ...
    // 延时创建渲染器，当用户只依赖响应式依赖包的时候，可以通过 tree-shaking 移除核心渲染相关逻辑的代码
    // ??? 疑问：具体是实现的
        ...
        if (!context.inVPre && /^(v-|:|\.|@|#)/.test(name)) {
            ...
            if (match[2]) {
                ...
                arg = {
                    ...
                    isStatic,
                    constType: isStatic ?
                        3 /* CAN_STRINGIFY */ : 0 /* NOT_CONSTANT */ ,
                    loc
                };
            }
